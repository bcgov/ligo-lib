...
from linker.linker_core.base import (link_config,
                                   CHUNK_SIZE,
                                   COLUMN_TYPES,
                                   _save_pairs,
                                   sort_csv)
from linker.linker_core.chunked_link_base import ChunkedLinkBase
...
class ChunkedDedup(ChunkedLinkBase):
    ...
    def extract_rows(self, data_filename, data_id, index_filename, index_id,
        ...
        with open(data_filename, 'r') as data_file, \
                open(index_filename, 'r') as index_file, \
                open(selected_filename, 'w') as selected_file, \
                open(remained_filename, 'w') as remained_file:

            data_reader = csv.reader(data_file)
            index_reader = csv.reader(index_file)

            selected_writer = csv.writer(selected_file, lineterminator='\n')
            remained_writer = csv.writer(remained_file, lineterminator='\n')
